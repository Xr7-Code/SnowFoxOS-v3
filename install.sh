...
check_void(){
    . /etc/os-release 2>/dev/null || error "Missing os-release"
    [[ "$ID" != "void" ]] && error "Not Void Linux"
    command -v xbps-install >/dev/null 2>&1 || error "xbps missing"
    success "Void Linux detected"
}

check_user(){
    REALUSER=${SUDO_USER:-$(logname 2>/dev/null)}
    [[ -z "$REALUSER" ]] && error "No user found"
    HOMEDIR="/home/$REALUSER"
    [[ ! -d "$HOMEDIR" ]] && error "Home missing"
    success "User: $REALUSER"
}

# -------------------- gpu --------------------

detect_gpu(){
    command -v lspci >/dev/null || xbps-install -Sy pciutils
    GPU="generic"
    lspci | grep -qi nvidia && GPU="nvidia"
    lspci | grep -qi amd && GPU="amd"
    lspci | grep -qi intel && GPU="intel"
    success "GPU: $GPU"
}
...
PACKAGES=(
    # X11 base
    xorg-minimal xinit xrandr xset xsetroot xclip xprop
    xf86-input-libinput
...
update_system(){
    info "System update"
    xbps-install -Syu -y || error "update failed"
}

install_packages(){
    info "Installing packages"
    for p in "${PACKAGES[@]}"; do
        xbps-install -S -y "$p" || warn "skip: $p"
    done
}
...
enable_services(){
    ...
    for s in dbus elogind NetworkManager tlp; do
        [[ -d /etc/sv/$s ]] && ln -sf /etc/sv/$s /var/service/ || warn "missing service: $s"
    done
}
...
setup_groups(){
    info "User groups"
    for g in wheel audio video input network storage; do
        getent group "$g" >/dev/null && usermod -aG "$g" "$REALUSER" || true
    done
}
...
install_configs(){
    ...
    local base
    base="$(dirname "$(realpath "$0")")/configs"
...
exec i3
EOF
    chown "$REALUSER:$REALUSER" "$HOMEDIR/.xinitrc"
}
...
install_snowfox_cli(){
    ...
    [[ -f "$dir/snowfox" ]] && cp "$dir/snowfox" /usr/local/bin/ && chmod +x /usr/local/bin/snowfox || warn "snowfox missing"
}
